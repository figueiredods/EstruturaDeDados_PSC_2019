/*Dijkstra's algorith on a graph represented using adjacency matrix*/
...
void readgraph()
{
	...
	for (i = 0; i<n; i++) //create adjacency matrix
		for (j = 0; j<n; j++)
			if (adj[i][j] != 0)
				insert(i, j, adj[i][j]);
...
void insert(int vi, int vj, int w)
{
	...
	q->next = NULL;
	q->weight = w;
	//insert the node in the linked matrix for the vertex no. vi
	if (G[vi] == NULL)
		G[vi] = q;
	else
	{
		// go to the end of linked matrix
		p = G[vi];
		while (p->next != NULL)
			p = p->next;
		p->next = q;
